...
class CsvExportChannelStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'code' => [
                ...
                'max:50',
                'regex:/^[A-Za-z0-9_-]+$/',
                Rule::unique('csv_export_channels', 'code')->where(function ($query): void {
            ...
            'name' => [
                ...
                'string',
                'max:100',
                Rule::unique('csv_export_channels', 'name')->where(function ($query): void {
                    $query->where('user_id', $this->user()?->id);
                }),
            ],
            'measurement' => ['required', 'string', 'max:100'],
            'timestamp_source' => ['nullable', 'string', Rule::in(['now', 'task_created_at', 'task_started_at', 'task_finished_at', 'task_updated_at'])],
